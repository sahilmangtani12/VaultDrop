...
async function handleDownload(transfer: Download) {
  switch (transfer.status) {
    case "preparing": {
      const allFiles: InodeMembers[] = [];
      const rootHandle = await getRootHandle(transfer.id);

      for (const mnemonic of transfer.mnemonics) {
        const { data: tree, error } = await api.fs.tree(mnemonic);
        if (!tree || error) {
          return toError(
            transfer,
            `Failed to list tree for ${mnemonic}`,
          );
        }
      ...
      const size = allFiles.reduce((acc, file) => {
        if (file.data) {
    ...
    case "finishing": {
      ...
      if (transfer.mnemonics.length === 1) {
        const mnemonic = transfer.mnemonics[0];
        const handle = handles.get(mnemonic);
        
        if (!handle) {
          return toError(transfer, "Failed to get handle");
        }
        handles.delete(mnemonic);

        if (handle.kind === "file") {
          const file = await (handle as FileSystemFileHandle).getFile();
          // Cleanup OPFS
          const opfsRoot = await navigator.storage.getDirectory();
          await opfsRoot.removeEntry(transfer.id, { recursive: true }).catch(() => {});
      ...
      // Zip generation for multiple files or directory
      const zip = new JSZip();
      await addToZip(rootHandle, "", zip);
      const blob = await zip.generateAsync({ type: "blob" });
      
      const zipName = transfer.mnemonics.length === 1 ? `${transfer.mnemonics[0]}.zip` : "archive.zip";
      const file = new File([blob], zipName);
      
      // Cleanup OPFS for this transfer
      const opfsRoot = await navigator.storage.getDirectory();
      await opfsRoot.removeEntry(transfer.id, { recursive: true }).catch(() => {});
